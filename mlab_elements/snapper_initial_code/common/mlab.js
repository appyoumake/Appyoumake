...
Mlab.prototype = {
     ...
     * value. If it gets the state from a remote location, it has to accept a callback function as argument, and it has
     * to set the first element of the array to true (if the remote call was actually made, that is).
     * @param {String} user User ID for the currently logged in user. Required.
     * @param {String} key Key name for the state to be stored. Required.
     * @return {Object} Object containing state's value (any type), or null, and any event name that we need to listen for.
     */
    getState: function(user, key, callback) {
        var value = this.internal.dispatchToPlugin("getState", user, key, callback);
/*
        var async = false;
        // getState is implemented in plugin. Unpack to see what we got.
...
*/
        ...
        if (typeof(callback)=="function") return callback(key, value);
        return value
    },
